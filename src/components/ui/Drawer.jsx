    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50">
          ...
          />
          <motion.div
            className={`fixed right-0 top-0 h-full bg-white ${width} max-w-full overflow-y-auto z-10`}
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-[#e5e7eb] sticky top-0 bg-white z-10">
              ...
              <button onClick={onClose} className="p-2 rounded-btn hover:bg-gray-100 text-gray-500">
                <X className="w-4 h-4" />
              </button>
            </div>
